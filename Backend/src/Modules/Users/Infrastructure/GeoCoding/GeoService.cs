...
namespace Users.Infrastructure.GeoCoding
{
    internal class GeoService : IGeoService
    {
        ...
        {
            double lat1 = double.Parse(coords1.Latitude) * (Math.PI / 180);
            double lat2 = double.Parse(coords2.Latitude) * (Math.PI / 180);
            double lon1 = double.Parse(coords1.Longitude) * (Math.PI / 180);
            double lon2 = double.Parse(coords1.Longitude) * (Math.PI / 180);
